...
int main() {
  ...
  for (;;) {
    ...
    printf("\n\n%s Pilihan? ", SPACE);
    scanf("%d", &choose);
    if (getchar() != '\n') { // proteksi inputan, mengencek apakah inputan
                             // megandung karakter seperti (1j,abc,dll)
      fflush(stdin);
      system("color 04");
...
void menuCrud() { // menu untuk CRUD
  ...
  while (1) {
    ...
    sort(&head, "asc",
         "nama"); // urutkan data secara ascending, berdasarkan nama
    if (pilihan == 1) {
      system("cls");
      createData();
      updateDB();
...
void createData() { // fungsi untuk create data pada CRUD
  ...
  srand(time(
      NULL)); // untuk membuat angka acak berdasarkan waktu yang selalu berubah
...
void deleteData() { // fungsi untuk menhapus data pada linked list
  ...
  temp = head;
  for (int i = 0; temp && i < get_data - 1;
       i++) // perulangan untuk menuju posisi data yg akan dihapus
    temp = temp->next;
...
void searchData() { // cari data pada CRUD, menggunakan metode Linier search
  ...
  while (ptr) {
    if (strcmpi(ptr->name, search) == 0 ||
      ...
      found = 1;
      // Kalau data ditemukan masukan ke linked list head_search
  ...
  if (found) // kalau misal data ditemukkan
    tabel(head_search);
  else {
    system("color 04");
    printf("\n%s  Data tidak ditemukkan!", SPACE);
  }
  free(head_search); // hapus linked list head_search
}
...
void line(char print[], int ascii1, int ascii2,
          int ascii3) { // untuk mencetak baris dalam membentuk tabel
  int index = 0;
    ...
    for (j = 0; j < count_list - 1; j++) {
      ...
      if (strcmpi(sort_mode, "asc") == 0) {
        if (strcmpi(field, "kode") == 0)
          do_swap = curr->kode > next->kode;    // kalau curr->kode > next->kode return 1, sebaliknya 0
...
void password_hash(char pass[]) { // membuat password menjadi bintang-bintang
  int key = 0, index = 0;
  do {
    key = getch(); // berfungsi untuk menerima inputan 1 karakter lalu konvert
                   // ke desimal
    switch (key) {
    case 0:
      getch(); // 0 merupakan desimal dari F1-F12, lalu bersihkan inputan di key
      break;
    case 224: // 255 merupakan desimal dari tombol home, end, dll, lalu
              // bersihkan inputan di key
      getch();
      break;
    case 8: // 8 desimal dari backspace, delete 1 karakter dan delete 1 bintang
      if (index > 0) {
        pass[index] = 0;
        index--;
        printf("\b \b");
      ...
      break;
    default:
      if (key >= 32 && key <= 126) { // karakter yang diterima
        pass[index] = key;
        index++;
        printf("*"); // kalau karater diterima, maka cetak bintang di console
      }
    }
  } while (key != 13);
  pass[index] = '\0';
...
void login() { // login
  ...
  for (;;) {
    ...
    for (int i = 0; i < kolom; i++) { // animasi bagian atas bawah
      // Atas
      gotoxy(space + i, 2);
      printf("=");
      // Bawah
    ...
    for (int i = 0; i < 9; i++) { // animasi bagian samping
...
memesan_menu:  // goto untuk mengulang pesanan
  system("color 71 && cls");
  header(
      "**PESAN MENU RESOTRAN**"); // panggil fungsi header untuk mencetak title
  if (!head) {
    system("color 74");
  ...
  printf("\n%sMasukkan kode menu yang dipilih: ",
         SPACE); // input untuk masukkan kode menu
  scanf("%d", &kode_menu);
...
peyakinan: // goto untuk mengulang peyakinan
  fflush(stdin);
  printf("\n%sApakah anda ingin memesan lagi?\n%s[input Y untuk Ya]\t[input T "
         "untuk Tidak] ",
         SPACE, SPACE); // ulang pesanan
  scanf("%s", &yakin);
  if (strcmpi("T", yakin) == 0) { // jika tidak ingin pesan lagi
    ...
    getch();
    system("cls && color 71");
    header("**PESAN MENU RESOTRAN**"); // panggil fungsi header untuk mencetak
                                       // title
    goto peyakinan;
  }
}
void penghitungan() { // menghitung harga pesanan
  int pilihan, harga_bersih, harga_total = 0, bayar, i, total_dibayarkan = 0;
bayar:
  system("cls && color F1");
  ...
  printf("\n%c %-7s%c %-34s%c %-14s%c %-10s%c %-14s%c %-10s%c %-15s%c\n", '|',
         "KODE", '|', "NAMA MENU", '|', "HARGA", '|', "DISKON(%)", '|',
         "KATEGORI", '|', "JUMLAH", '|', "HARGA TOTAL", '|');
  menu_t *curr = head_pesanan;
  while (curr) {
    for (i = 0; i < 119; i++)
      printf("=");
    harga_bersih =
        (curr->harga - (curr->harga * curr->diskon / 100)) * curr->jumlah_beli;
    printf(
        "\n%c %-7d%c %-34s%c Rp.%-11d%c %-9d %c %-14s%c %-10d%c Rp.%-12d%c\n",
        '|', curr->kode, '|', curr->name, '|', curr->harga, '|', curr->diskon,
        '|', curr->kategori, '|', curr->jumlah_beli, '|', harga_bersih);
    curr = curr->next;
  }
  for (i = 0; i < 119; i++)
    printf("=");
  ...
  if (pilihan == 1) {
    ...
    temp = head_pesanan;
    while (temp) {
      harga_bersih = (temp->harga - (temp->harga * temp->diskon / 100)) *
                     temp->jumlah_beli;
      harga_total += harga_bersih;
      temp = temp->next;
    }
    printf("\n\n%sHarga yang harus dibayar Rp. %d\n", SPACE,
           harga_total - total_dibayarkan);
    printf("%sMasukkan uang: ", SPACE);
    scanf("%d", &bayar);
    total_dibayarkan += bayar;
    if (total_dibayarkan >= harga_total) {
      system("color F2");
      printf("%sPembayaran sukses", SPACE);
      strukPembayaran(harga_total, total_dibayarkan);
      head_pesanan = NULL;
      system("cls && color 70");
      ...
      system("color F4");
      printf("%sUang Anda tidak cukup! Silakan isi kekurangan sebanyak: Rp.%d",
             SPACE, harga_total - total_dibayarkan);
      getch();
      harga_total = 0;
      goto bayar;
    }
...
void strukPembayaran(
    int harga_total,
    int bayar) { // cetak struk pebayaran menggunakan file handling
  int harga_bersih, i;
  time_t ambil_waktu;
  time(&ambil_waktu);
